...
import Logger
import ch7.Token.Companion.tok_binary
import ch7.Token.Companion.tok_def
import ch7.Token.Companion.tok_else
import ch7.Token.Companion.tok_eof
import ch7.Token.Companion.tok_extern
import ch7.Token.Companion.tok_for
import ch7.Token.Companion.tok_identifier
import ch7.Token.Companion.tok_if
import ch7.Token.Companion.tok_in
import ch7.Token.Companion.tok_number
import ch7.Token.Companion.tok_then
import ch7.Token.Companion.tok_unary
import ch7.Token.Companion.tok_var
import isAscii
...
private fun createEntryBlockAlloca(theFunction: LLVMValueRef?, varName: String): LLVMValueRef? {
    val builder = LLVMCreateBuilderInContext(theContext)
    val entryBasicBlock = LLVMGetEntryBasicBlock(theFunction)
    val firstInstruction = LLVMGetFirstInstruction(entryBasicBlock)
    LLVMPositionBuilder(builder, entryBasicBlock, firstInstruction)
...
private data class VariableExprAST(val name: String) : ExprAST {
    override fun codegen(): LLVMValueRef? {
        // Look this variable up in the function
        val v = namedValues[name] ?: logErrorV("Unknown variable name")
        // Load the value
        return LLVMBuildLoad2(builder, LLVMDoubleTypeInContext(theContext), v, name)
    }
}
...
private data class BinaryExprAST(private val op: Char, private val LHS: ExprAST, private val RHS: ExprAST) : ExprAST {
    override fun codegen(): LLVMValueRef? {
        ...
        if (op == '=') {
            ...
            if (LHS !is VariableExprAST) return logErrorV("destination of '=' must be a variable")

            // Codegen the RHS.
            val arg = RHS.codegen() ?: return null
            // Look up the name.

            val variable = namedValues[LHS.name] ?: logErrorV("Unknown variable name")

            LLVMBuildStore(builder, arg, variable)
            return arg
        }
...
private data class ForExprAST(
    ...
    override fun codegen(): LLVMValueRef? {
        ...
        val alloca = createEntryBlockAlloca(function, varName)
        ...
        val curVar = LLVMBuildLoad2(builder, LLVMDoubleTypeInContext(theContext), alloca, "curValue")
        val nextVar = LLVMBuildFAdd(builder, curVar, step, "nextvar")
...
private data class VarExprAST(private val varNames: List<Pair<String, ExprAST?>>, private val body: ExprAST) : ExprAST {
    override fun codegen(): LLVMValueRef? {
        val oldBindings = mutableListOf<LLVMValueRef?>()

        val theFunction = LLVMGetBasicBlockParent(LLVMGetInsertBlock(builder))

        // Register all variables and emit their initializer.
        varNames.forEach { (varName, init) ->
            // Emit the initializer before adding the variable to scope, this prevents
            // the initializer from referencing the variable itself, and permits stuff
            // like this:
            //  var a = 1 in
            //    var a = a in ...   # refers to outer 'a'.
            val initVal = if (init != null) {
                init.codegen() ?: return null
            } else {
                // If not specified, use 0.0.
                LLVMConstReal(LLVMDoubleTypeInContext(theContext), 0.0)
            }
            val alloca = createEntryBlockAlloca(theFunction, varName)
            LLVMBuildStore(builder, initVal, alloca)

            // Remember the old variable binding so that we can restore the binding when
            // we unrecurse.
            oldBindings += namedValues[varName]
        ...
        val bodyVal = body.codegen() ?: return null

        // Pop all our variables from scope.
        varNames.forEachIndexed { i, (name, _) ->
    ...
    fun codegen(): LLVMValueRef? {
        ...
        memScoped {
            ...
            for (i in 0..proto.args.lastIndex) {
                val argName = LLVMGetValueName2(array[i], lengthPtr)?.toKString()
                    ?: return logErrorV("Cannot get the name of variable")
                // Create an alloca for this variable.
                val alloca = createEntryBlockAlloca(theFunction, argName)

                // Store the initial value into the alloca.
                LLVMBuildStore(builder, array[i], alloca)
...
@ExperimentalUnsignedTypes
private fun parseVarExpr(): ExprAST? {
    getNextToken() // eat identifier.

    val varNames = mutableListOf<Pair<String, ExprAST?>>()

    if (curTok != tok_identifier) return logError("expected identifier after var")

    while (true) {
        val name = identifierString
        getNextToken() // eat identifier
        ...
        if (curTok == '='.toInt()) {
            ...
            init = parseExpression() ?: return null
        } else {
            init = null
        }
        varNames += name to init

        // End of var list, exit loop.
        if (curTok != ','.toInt()) break
        getNextToken() // eat the ','.
